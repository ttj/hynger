function [out] = daikon_dtrace_write_trace(model_block_name_daikon, simTime, simData, opt_dataflow, opt_multi, opt_time)
    ...
    if strcmp(simData(1).eventType, 'PreOutputs')
        ppt_name = [model_block_name_daikon, ':::ENTER'];
        i_ppt = iotype_input;
        output_filename_ppt = 'enter';
    elseif strcmp(simData(1).eventType, 'PostOutputs')
        ppt_name = [model_block_name_daikon, ':::EXIT0'];
        i_ppt = iotype_output;
        output_filename_ppt = 'exit';
    end
    ...
    if opt_multi
        ...
        %    end
        %else
            output_filename = ['output_', simTimeRep, '_', model_block_name_daikon, '_', output_filename_ppt, '.dtrace'];
        %end
